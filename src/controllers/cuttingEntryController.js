...
export const completeCuttingTrn = async (req, res, next) => {
    try {
        const entries = await cuttingService.completeCuttingTrn(req.params.trnNo);
        res.status(200).json({
            success: true,
            message: 'Cutting entries saved successfully',
            data: entries,
        });
    } catch (error) {
        next(error);
